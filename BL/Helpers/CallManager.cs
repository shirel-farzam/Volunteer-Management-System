...
internal static class CallManager
{
    ...
    public static BO.CallInProgress GetCallInProgress(int VolunteerId)
    {
        lock (AdminManager.BlMutex)
        {
            DO.Volunteer? doVolunteer = _dal.Volunteer.Read(VolunteerId) ?? throw new BlDoesNotExistException("eroor id");
    ...
    public static BO.CallAssignmentInList GetCallAssignInList(int Id)
    {
        lock (AdminManager.BlMutex)
        {
            DO.Volunteer? doVolunteer = _dal.Volunteer.Read(Id) ?? throw new BlDoesNotExistException("error id");
    ...
    {
        lock (LockObject)  // נעילה למניעת גישה חפופה לשיטת השליפה
        {
    ...
    {
        lock (LockObject)  // נעילה כאן לוודא שמטפלים במידע של קריאה בהקשר של הקצותה
        {
            // Find the appropriate CALL  and  Assignment by volunteer ID
            var doAssignment = _dal.Assignment.ReadAll().Where(a => a.VolunteerId == Id && a.TimeEnd == null).FirstOrDefault();
            var doCall = _dal.Call.ReadAll().Where(c => c.Id == /*doAssignment*/Id).FirstOrDefault();
    ...
    {
        lock (LockObject)  // נעילה על מנת למנוע עדכון מקביל של הקריאה
        {
    ...
    {
        lock (LockObject)  // נעילה כדי למנוע התנגשות על שליפת שמות מתנדבים
        {
    ...
    {
        lock (LockObject)  // נעילה כדי למנוע התנגשות בעדכון/שליפה של הזמן
        {
            // Retrieve all assignments related to the call
            var assignments = _dal.Assignment.ReadAll().Where(a => a.CallId == callId);
    ...
    {
        lock (LockObject)  // נעילה כדי למנוע התנגשות בעת החזרה
        {
            if (doCall == null)
            {
    ...
    {
        lock (LockObject)  // נעילה כדי להימנע ממספר תהליכים מעדכנים את ההקצותות בו זמנית
        {
